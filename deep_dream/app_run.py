...
def get_model():
    """download model"""
    model = os.path.join("deep_dream/model", model_name)
...
def run_app(input_path,output_path):
    t0 = time.time()
    image = np.float32(cv2.imread(input_path))
    if len(image.shape) >= 3 and image.shape[2] > 3:
        image = image[:,:,0:3]
    max_l = max(image.shape[0], image.shape[1])
    if max_l > 500:
        ratio = 500 / max_l
    ...
    deep_dream(model_path,output_path, input_image=image)
    t1 = time.time()
    print("用时{}s".format(t1 - t0))
